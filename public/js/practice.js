...
function renderGlobalStats(sections) {
  ...
  const bestScores = sections
    .map(s => stats[s.id]?.bestScore)
    .filter(Boolean)
    .map(b => Math.round((b.correct / b.total) * 100));
  ...
  el.innerHTML = `
    ...
    ${avgBest !== null ? `
    <div class="glass rounded-xl px-4 py-2 text-sm text-center">
      <span class="text-emerald-400 font-bold">${avgBest}%</span>
      <span class="text-slate-500 ml-1">avg best score</span>
    </div>` : ''}
  `;
}
...
function renderSectionCards(sections) {
  ...
  sections.forEach((section, i) => {
    ...
    const pctColor = pct === null ? '' : pct >= 80 ? 'text-emerald-400' : pct >= 50 ? 'text-yellow-400' : 'text-red-400';

    const bestLabel = best
      ? `<span class="${pctColor} font-semibold">${best.correct}/${best.total}</span> <span class="text-slate-500">best</span>`
  ...
  // Inject CSS that Tailwind CDN won't pre-scan
